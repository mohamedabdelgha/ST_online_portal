...
export default function Groups() {
  ...
  async function submit(e) {
    e.preventDefault()
    if (!editing && !isAdmin) { setError('Only administrators can create groups.'); return }
    try {
  ...
  return <div>
    <div className="page-heading"><div><h1>{isAdmin ? 'All Groups' : 'My Groups'}</h1><p>{isAdmin ? 'View and edit every group.' : 'Manage only the groups assigned to you.'}</p></div>{isAdmin && <button className="btn" onClick={startCreate}>+ New group</button>}</div>
